...
const MapContainer = ({ routingData }) => {
  ...
  const initializeMap = () => {
    ...
    const dataToUse = routingData && routingData.length > 0 ? routingData : storedRoutingData;

    const initialCenter = dataToUse && dataToUse.length > 0 ? 
    ...
    if (dataToUse && dataToUse.length > 0) {
      googleMap.current.setCenter(new window.google.maps.LatLng(dataToUse[0].latitude, dataToUse[0].longitude));
    }
  };
